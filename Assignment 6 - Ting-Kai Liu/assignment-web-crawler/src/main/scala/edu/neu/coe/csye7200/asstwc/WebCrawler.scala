...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] =
      for(n <- ns\\"a"; h <- n\"@href"; x <- h.head) yield createURL(Some(u), x.toString())
    // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (s <- getURLContent(u); data <- MonadOps.asFuture(getLinks(s))) yield data
    // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val values = for (u <- us2) yield wget(u)
    val future = for(usf <- values) yield MonadOps.sequence(usf)
